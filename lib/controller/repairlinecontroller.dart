...
class RepairLineController {
  Future<void> insertLines(BuildContext context, RepairLines lines,
      String idrecambio, int cantidad) async {
    DatabaseSqlite db = DatabaseSqlite();
    Database database = await db.openDB();

    try {
      var resultSet = await database
          .rawQuery("SELECT stock FROM Recambios WHERE id = ?", [idrecambio]);
      // Get first result
      ...
      if (stock >= cantidad) {
        ...
        await database.rawUpdate("UPDATE Recambios SET stock = ? WHERE id = ?",
            [stock, idrecambio]); //se actualiza el stock de reacmbios

      } else {
        String error =
            'No hay suficiente stock para este cantidad del recambio';
  ...
  Future<void> updateLine(BuildContext context, String idorden, String idlinea,
      String idrecambio, int cantidadold, int cantidadnew) async {
    DatabaseSqlite db = DatabaseSqlite();
    Database database = await db.openDB();
    ...
    try {
      ...
      var dbItem = resultSet.first;
      // Access its id
      var stock = dbItem['stock']
          as int; //saco el stock que hay ahora mismo en el recambio
      ...
      if (stock >= cantidadnew) {
        ...
        stock = stock -
            cantidadnew; //ahora le resto a stock cantidadnew que es la nueva cantidad introducida al actualizar

        await database.rawUpdate(
            "UPDATE LineasReparacion SET cantidad = ? WHERE idorden = ? and idlinea = ?",
            [
              cantidadnew,
              idorden,
              idlinea
            ]); //actualizo la linea con la nueva cantidad
  ...
  Future<void> deleteLines(
      String idorden, String idlinea, String idrecambio, int cantidad) async {
    DatabaseSqlite db = DatabaseSqlite();
    Database database = await db.openDB();

    var resultSet = await database.rawQuery(
        "SELECT stock FROM Recambios WHERE id = ?",
        [idrecambio]); //saco el stock actual del recambio
    // Get first result
    var dbItem = resultSet.first;
    // Access its id
    var stock = dbItem['stock'] as int;
    ...
    stock = stock +
        cantidad; //nuevo stock despues de borrar. Al stock actual le sumo el del recambio que tenia esta linea

    await database.rawUpdate("UPDATE Recambios SET stock = ? WHERE id = ?",
        [stock, idrecambio]); //se actualiza el stock de reacmbios

    await database.rawDelete(
        "DELETE FROM  LineasReparacion WHERE idorden= ? AND idlinea = ?",
        [idorden, idlinea]);
  }
  ...
  Future<List<RepairLines>> getLines(String idorden) async {
    ...
    final List<Map<String, dynamic>> maps = await database.rawQuery(
        "SELECT * FROM LineasReparacion WHERE idorden = ?", [idorden]);
